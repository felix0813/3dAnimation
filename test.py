...
VIEW = np.array([-0.8, 0.8, -0.8, 0.8, 1.0, 5.0])  # 视景体的left/right/bottom/top/near/far六个面
SCALE_K = np.array([1.0, 1.0, 1.0])  # 模型缩放比例
EYE = np.array([2.0, 2.0, 2.0])  # 眼睛的位置（默认z轴的正方向）
LOOK_AT = np.array([0.0, 0.0, 0.0])  # 瞄准方向的参考点（默认在坐标原点）
EYE_UP = np.array([0.0, 0.0, 1.0])  # 定义对观察者而言的上方（默认y轴的正方向）
WIN_W, WIN_H = 1920, 1080  # 保存窗口宽度和高度的变量
...
def draw_triangle():
    ...
    # glLoadIdentity()
    texture = TextureLoader.load_texture('D:\pythonProject\\textures\ground.jpg', glGenTextures(1))
...
if __name__ == "__main__":
    ...
    init()  # 初始化画布
    glutDisplayFunc(draw_triangle)  # 注册回调函数draw()
